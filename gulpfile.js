...
const paths = {
	...
	media: {
		src: `${dirs.entry}/media/**/*.+(gif|jpg|jpeg|png|svg|mp4)`,
	...
	scripts: {
		src: [
			`${dirs.entry}/scripts/**/*.+(js|php)`,
			`!${dirs.entry}/scripts/no-bundle/**/*`,
		],
...
const pluginConfig = {
	...
	nunjucksRender: {
		...
		data: {
			...
			paths: {
				root: isProd ? "https://westcovinamc.com" : ".",
...
//     gulp
//     // Input.
//     .src(dirs.entry + '/scripts/**/*.js')
//     .pipe(copy(dirs.output +'/static/scripts/'))
// );
...
gulp.task("default", (callback) => {
	const compile = ["clean", "public", "media", "views", "styles", "scripts"];
	if (isProd) {
		// Production.
